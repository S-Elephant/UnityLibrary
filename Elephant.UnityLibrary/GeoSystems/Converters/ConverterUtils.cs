using System;
using Elephant.UnityLibrary.GeoSystems.Converters;
using Elephant.UnityLibrary.GeoSystems.Validators;
...
namespace Elephant.GeoSystems.Converters
{
	...
	public static class ConverterUtils
	{
		...
		public static (float Latitude, float Longitude) OsmTileToGpsAsFloat(int zoom, int tileX, int tileY)
		{
			...
			double tileWidth = 2d * ConverterConstants.HalfEarthCircumference / (1 << zoom);
			// The height of a tile is the same as the width because the tiles in Open Street Map are square.
			double tileHeight = 2d * ConverterConstants.HalfEarthCircumference / (1 << zoom);
			...
			double centerX = (tileX + 0.5d) * tileWidth - ConverterConstants.HalfEarthCircumference;
			// Calculate the vertical distance in meters from the top-most edge of the top-most tile to the center of the given tileY.
			double centerY = ConverterConstants.HalfEarthCircumference - (tileY + 0.5d) * tileHeight;
			...
			double longitude = centerX / ConverterConstants.EarthRadius * 180d / Math.PI;
			...
			double latitude = (2d * Math.Atan(Math.Exp(centerY / ConverterConstants.EarthRadius)) - Math.PI / 2d) * 180d / Math.PI;
		...
		public static (decimal Latitude, decimal Longitude) OsmTileToGpsAsDecimal(int zoom, int tileX, int tileY)
		{
			// Calculate the width of a single tile at the given zoom level in meters.
			// The total width (2*HalfEarthCircumference) is divided by the number of tiles, which is 2^zoom.
			decimal tileWidth = 2m * ConverterConstants.HalfEarthCircumferenceAsDecimal / (1 << zoom);
			// The height of a tile is the same as the width because the tiles in Open Street Map are square.
			decimal tileHeight = 2m * ConverterConstants.HalfEarthCircumferenceAsDecimal / (1 << zoom);

			// Calculate the horizontal distance in meters from the left-most edge of the left-most tile to the center of the given tileX.
			decimal centerX = (tileX + 0.5m) * tileWidth - ConverterConstants.HalfEarthCircumferenceAsDecimal;
			// Calculate the vertical distance in meters from the top-most edge of the top-most tile to the center of the given tileY.
			decimal centerY = ConverterConstants.HalfEarthCircumferenceAsDecimal - (tileY + 0.5m) * tileHeight;

			// Convert the centerX value to degrees to get the longitude.
			// The formula derives from the relationship between arc length and angle in a circle (arc length = angle * radius).
			decimal longitude = centerX / ConverterConstants.EarthRadiusAsDecimal * 180m / ConverterConstants.PiAsDecimal;

			// Convert the centerY value to degrees to get the latitude.
			// The formula involves some trigonometry and is used to transform Mercator-projected Y coordinate back to latitude.
			decimal latitude = (2m * Atan(Exp(centerY / ConverterConstants.EarthRadiusAsDecimal)) - ConverterConstants.PiAsDecimal / 2m) * 180m / ConverterConstants.PiAsDecimal;
		...
		public static (double X, double Y) GpsToRd(double latitudeWgs84, double longitudeWgs84)
		{
			...
			double latitude = 0.36d * (latitudeWgs84 - ConverterConstants.AmersfoortWgs84CoordinateX);
			double longitude = 0.36d * (longitudeWgs84 - ConverterConstants.AmersfoortWgs84CoordinateY);
			...
			double rdX = ConverterConstants.AmersfoortRdCoordinateX + latitudeCalculation;
			double rdY = ConverterConstants.AmersfoortRdCoordinateY + longitudeCalculation;
		...
		public static (double Latitude, double Longitude) RdToGps(double rdX, double rdY)
		{
			...
			double deltaX = (rdX - ConverterConstants.AmersfoortRdCoordinateX) * 0.00001d; //// 0.00001 = Math.Pow(10, -5);
			double deltaY = (rdY - ConverterConstants.AmersfoortRdCoordinateY) * 0.00001d; //// 0.00001 = Math.Pow(10, -5);
			...
			double latitude = ConverterConstants.AmersfoortWgs84CoordinateX + (sumN / 3600d);
			double longitude = ConverterConstants.AmersfoortWgs84CoordinateY + (sumE / 3600d);
		...
		{
			// Convert RD coordinates to a delta relative to Amersfoort, then scale down by 10^5.
			double deltaX = ((double)rdX - ConverterConstants.AmersfoortRdCoordinateX) * 0.00001d; //// 0.00001 = Math.Pow(10, -5);
			double deltaY = ((double)rdY - ConverterConstants.AmersfoortRdCoordinateY) * 0.00001d; //// 0.00001 = Math.Pow(10, -5);
