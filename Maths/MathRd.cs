...
namespace Elephant.UnityLibrary.Maths
{
	...
	public static class MathRd
	{
		...
		public static (float Latitude, float Longitude) ConvertToLatitudeLongitude(float x, float y, bool allowInvalidCoordinate = false)
		{
			...
			if (!allowInvalidCoordinate && (!IsValidLatitude(latitude) || !IsValidLongitude(longitude)))
			{
				Debug.LogError($"Got invalid GPS coordinate. RDS: {x},{y}, GPS: {latitude},{longitude}");
			}
		...
		/// <summary>
		/// Convert a RD (=Rijksdriehoek) coordinate into a WGS84 GPS coordinate.
		/// </summary>
		/// <param name="rds">RDS coordinates.</param>
		public static Vector2 ConvertToLatitudeLongitude(Vector2 rds)
		{
			(float Latitude, float Longitude) result = ConvertToLatitudeLongitude(rds.x, rds.y);
			return new Vector2(result.Latitude, result.Longitude);
		}

		/// <summary>
		/// Convert a GPS coordinate into a RD (=Rijksdriehoek) coordinate.
		/// </summary>
		/// <param name="gps">GPS coordinates.</param>
		public static Vector2 ConvertToRds(Vector2 gps)
		{
			(float X, float Y) result = ConvertToRijksdriehoek(gps.x, gps.y);
			return new Vector2(result.X, result.Y);
		}

		/// <summary>
		/// Returns true if <paramref name="gpsCoordinate"/> is a valid GPS coordinate.
		/// </summary>
		public static bool IsValidLatitude(Vector2 gpsCoordinate)
		{
			return IsValidLatitude(gpsCoordinate.x) && IsValidLongitude(gpsCoordinate.y);
		}

		/// <summary>
		/// Returns true if <paramref name="latitude"/> is a valid GPS latitude.
		/// </summary>
		public static bool IsValidLatitude(float latitude)
		{
			return latitude >= (-90f - float.Epsilon) && latitude <= (90f + float.Epsilon);
		}

		/// <summary>
		/// Returns true if <paramref name="longitude"/> is a valid GPS longitude.
		/// </summary>
		public static bool IsValidLongitude(float longitude)
		{
			return longitude >= (-180f - float.Epsilon) && longitude <= (180f + float.Epsilon);
		}
